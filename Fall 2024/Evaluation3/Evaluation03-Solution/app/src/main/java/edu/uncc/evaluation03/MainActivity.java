...
public class MainActivity extends AppCompatActivity implements BudgetListFragment.BudgetListListener, ExpenseSummaryFragment.ExpenseSummaryListener,
        SelectCategoryFragment.SelectCategoryListener, SelectPriorityFragment.SelectPriorityListener, AddExpenseFragment.AddExpenseListener {
    ...
    public void gotoExpenseSummary(Expense expense) {
        double totalForCat = 0.0;
        for (Expense item: mExpenses) {
            if(item.getCategory().equals(expense.getCategory())){
                totalForCat = totalForCat + item.getAmount();
            }
        }
        ...
        getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.main, ExpenseSummaryFragment.newInstance(expense, totalForCat))
                .addToBackStack(null)
                .commit();
    }

    @Override
    public void onCancelExpenseSummary() {
        getSupportFragmentManager().popBackStack();
    }
}